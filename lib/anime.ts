...
export function voiceoverEpisodeIndices(
  episodes: Episode[],
  voiceover?: Voiceover,
) {
  const ordinals = voiceover?.episode_ordinals
    ? new Set(voiceover.episode_ordinals)
    : null;
  return episodes.flatMap((episode, index) =>
    (
      ordinals
        ? ordinals.has(episode.ordinal)
        : episode.ordinal <= (voiceover?.episodes || 0)
    )
      ? [index]
      : [],
  );
...
export function initialVoiceover(voiceovers: Voiceover[], episode: number) {
  const available = (v: Voiceover) =>
    v.episode_ordinals
      ? v.episode_ordinals.includes(episode)
      : v.episodes >= episode;
