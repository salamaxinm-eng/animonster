import { ApiError, body, db, fail, json, now, requireUser, sameOrigin, uid } from '@/lib/server/core';
...
export async function DELETE(r: Request) {
  try {
    ...
    if (b.confirmation !== 'УДАЛИТЬ') throw new ApiError('Введите УДАЛИТЬ для подтверждения.');
    ...
    await db().batch([
      ...
      db().prepare('UPDATE users SET identity=?,email=NULL,password_hash=NULL,nick=?,bio=?,avatar=?,pin=NULL,adult_confirmed_at=NULL,deleted_at=? WHERE id=?')
        .bind(anonymous, `Удалённый_${user.id.slice(0, 8)}`, '', 'moon', now(), user.id),
    ]);
    return json({ ok: true });
  } catch (e) { return fail(e); }
}
