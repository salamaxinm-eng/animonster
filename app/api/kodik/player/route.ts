...
export async function GET(request: Request) {
  try {
    ...
    const access = await db()
      .prepare(
        `SELECT free_at
         FROM anime_episode_availability
    ...
    if (access && Number(access.free_at) > now()) {
      if (!user || (await premium(user.id)) <= now()) {
        throw new ApiError(
          'Серия доступна по AniMonster Plus',
          403,
        );
      }
    }
    ...
    const source = await db()
      .prepare(
        `SELECT player_url, episodes_count
         FROM anime_sources
         WHERE provider='kodik'
           AND anime_id=?
           AND translation_id=?
           AND active=1
    ...
    if (!source) {
      throw new ApiError(
        'Серия временно недоступна',
        404,
      );
    }

    if (episode > Number(source.episodes_count || 0)) {
      throw new ApiError(
        'Серия временно недоступна',
        404,
      );
    ...
    const start = Math.floor(
      Number(params.get('start_from')) || 0,
    );
